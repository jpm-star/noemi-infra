...
def procurar_por_nome(nome: str, cidade: str = "", uf: str = "", *,
    ...
    recs = _fetch(nucleo, limite) if _fetch else _fetch_cnpja(nucleo, limite, timeout)
    if recs is None:
...
def _fetch_cnpja(nucleo: str, limite: int, timeout: float, _sleep=None):
    """GET /office?names.in=<núcleo>. Retorna records[] ou None em erro. Key do env.
    ...
    qs = urllib.parse.urlencode({"names.in": nucleo[:40], "limit": str(limite)})
